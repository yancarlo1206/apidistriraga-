...
public class CotizacionTipoController {
	...
	private CotizacionTipoService cotizacionTipoService;
	 ...
	 @PostMapping
		public ResponseEntity<Responses<?>> guardar(@Valid @RequestBody CotizacionTipo request, BindingResult bindingResult) {
			...
			try {
				...
				return ResponseEntity.ok(new Responses<>("Tipo de cotizacion creado correctamente", HttpStatus.OK.value(), creado));
			} catch (Exception e) {
		...
		public ResponseEntity<String> eliminar(@PathVariable Integer id) {
			...
			if (eliminado) {
				return ResponseEntity.ok("Tipo de cotizacion con ID:" + id + " eliminado correctamente");
			} else {
				return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Tipo de cotizacion con ID:" + id + "no encontrado");
			}
